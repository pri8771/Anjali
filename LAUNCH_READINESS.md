...
- **Tested (UI, XCTest, 1 suite, `AnjaliUITests`):** `AnjaliFullLoopUITests` — the primary user journey end-to-end on a real simulator: launch → onboarding (welcome + preferences → Enter) → Today → begin a prayer in Silent mode → Complete → completion screen → back to Today; plus a dedicated case driving the two audio-less prayers (`vishnu-shantakaram`, `hanuman-manojavam`) through Moments → Deity → Vishnu → Listen, asserting the "Audio isn't available" fallback message and running timer rather than a crash. Verified locally (`xcodebuild test`) 2026-07-27: 56 unit + 2 UI tests, 0 failures.
- **Not tested / not automatable here:** most SwiftUI views beyond the one journey above (still no snapshot tests), `NotificationManager` scheduling, SwiftData persistence and the `MeView.deleteSaved` path, onboarding permission branching (reminders opted in), deep-link → coordinator → navigation end-to-end, and every accessibility/Dynamic-Type behaviour. CI now also runs a macOS `build-and-test` job (`.github/workflows/ci.yml`) executing the full Xcode build + unit + UI test suite on a discovered iPhone simulator, alongside the pre-existing Ubuntu content-validation job.
