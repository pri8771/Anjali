...
## 08. Execution
Plan: (1) build + test on macOS (`./Scripts/build.sh`) — done 2026-07-27, build succeeds, 56 unit + 2 UI tests pass; (2) run the `SETUP.md` smoke test; (3) close the content-provenance gate (named human reviewer sign-off enforced in data + validator, not the hardcoded `"seed"`); (4) add the real app icon; (5) add a `PrivacyInfo.xcprivacy` manifest; (6) verify accessibility on device; (7) decide whether to trim the Me tab; (8) screenshots + App Store Connect metadata; (9) archive → TestFlight → submit. The ordered, percentage-tracked path is in [LAUNCH_READINESS.md §8](../LAUNCH_READINESS.md).

## 09. QA
Tested (XCTest, 6 unit suites + 1 UI suite): the Today selection engine, time-band resolver, content loader/validation, prayer model, theme palette, deep links, and (new) `AnjaliUITests` covering the onboarding → Today → player → completion loop plus the two audio-less prayers' fallback behaviour end-to-end on a simulator. **Not yet tested:** most SwiftUI views beyond that one journey (still no snapshot tests), notification scheduling, SwiftData persistence, onboarding permission branching (reminders opted in), and all accessibility behaviour. CI runs content validation (Ubuntu) plus a macOS `build-and-test` job (`.github/workflows/ci.yml`) that builds and runs the full unit + UI test suite. Manual QA must cover: ritual start/completion, saved prayers, reminder permission, offline content, reset state, Dynamic Type, VoiceOver, Reduce Motion, and contrast on all five bands. See [LAUNCH_READINESS.md §8 test-coverage summary](../LAUNCH_READINESS.md).
